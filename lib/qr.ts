...
// ── Session QR token (generated by lecturer, scanned by student) ──────────────

const SESSION_QR_TTL = 30; // seconds
...
export function signSessionQRToken(sessionId: string): string {
  const full: SessionQRPayload = {
    type: 'session-qr',
    sessionId,
    nonce: crypto.randomBytes(8).toString('hex'),
    exp: Math.floor(Date.now() / 1000) + SESSION_QR_TTL,
  };
  return sign(full);
}

export function verifySessionQRToken(token: string): SessionQRPayload {
  const raw = verify(token);
  if (raw.type !== 'session-qr') throw new Error('Wrong token type');
  return raw as unknown as SessionQRPayload;
}

export { SESSION_QR_TTL };

// ── Token type detection ──────────────────────────────────────────────────────

export function detectTokenType(token: string): 'attendance' | 'challenge' | 'session-qr' | 'invalid' {
  try {
    const dotIndex = token.lastIndexOf('.');
    ...
    if (raw.type === 'session-qr') return 'session-qr';
    return 'attendance';
  } catch {
    return 'invalid';
  }
