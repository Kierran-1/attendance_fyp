...
async function main() {
  ...
  // Create or get unit (course)
  const unitCode = 'COS20031';
  const unitName = 'Database Design Project';
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    // Parse class info - convert LA1, LA2 to proper SessionType
    let sessionTypeRaw = 'LECTURE';
    let groupNumber = '01';
    let dayOfWeek = 'Mon';
    let startTime = '10:00';
    let endTime = '12:00';
    ...
    if (classInfo && classInfo.includes(',')) {
      ...
      if (parts.length >= 5) {
        sessionTypeRaw = parts[0] || 'LECTURE';
    ...
    // Convert LA1, LA2, LE1 to valid SessionType
    let sessionType: 'LECTURE' | 'TUTORIAL' | 'LAB' | 'PRACTICAL' = 'LECTURE';
    if (sessionTypeRaw.startsWith('LA')) {
      sessionType = 'LAB';
    } else if (sessionTypeRaw.startsWith('LE')) {
      sessionType = 'LECTURE';
    } else if (sessionTypeRaw.startsWith('TU')) {
      sessionType = 'TUTORIAL';
    } else if (sessionTypeRaw.startsWith('PR')) {
      sessionType = 'PRACTICAL';
    }
    
    console.log(`Class: ${sessionTypeRaw} -> ${sessionType}, Group: ${groupNumber}, Day: ${dayOfWeek}`);
    ...
    for (let i = headerRowIndex + 2; i < data.length; i++) {
      ...
      const nationality = row[6]?.toString() || 'Malaysian';
      ...
      for (const dateCol of dateColumns) {
        ...
        if (status && (status === 'P' || status === 'A' || status === 'p' || status === 'a')) {
          ...
          const attendanceDate = new Date(2026, parseInt(month) - 1, parseInt(day));
    ...
    console.log(`Processed ${studentCount} students in ${sheetName}`);
  }
